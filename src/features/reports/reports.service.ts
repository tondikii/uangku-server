...
export class ReportsService {
  ...
  async getMonthlyReport(user: User, year: number, month: number) {
    ...
    const categoryRaw = await this.transactionRepo
      .createQueryBuilder('transaction')
      ...
      .andWhere('type.id = 2') // ONLY REAL EXPENSE
      .select([
    ...
    const totalExpenseForBreakdown = expense; // includes admin fee

    const categories = categoryRaw.map((item) => {
      ...
      return {
        ...
        percentage:
          totalExpenseForBreakdown > 0
            ? Number(((total / totalExpenseForBreakdown) * 100).toFixed(2))
            : 0,
      };
    });
    ...
    const adminFeePercentage =
      totalExpenseForBreakdown > 0
        ? Number(((adminFee / totalExpenseForBreakdown) * 100).toFixed(2))
        : 0;
